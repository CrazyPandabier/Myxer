...
impl Meters {
    ...
    pub fn load_config(
        &mut self,
    ) -> Result<(std::ffi::OsString, String), Box<dyn std::error::Error>> {
        let config_path = self._config_path.clone();
        ...
        for line in config_content.lines() {
            ...
            match key {
                "show_visualizers" => {
                    self.show_visualizers = value.parse().unwrap_or(self.show_visualizers)
                }
                "separate_channels" => {
                    self.separate_channels = value.parse().unwrap_or(self.separate_channels)
                }
                "remember_position" => {
                    self.remember_position = value.parse().unwrap_or(self.remember_position)
                }
                "window_position_x" => {
                    self.window_position.0 = value.parse().unwrap_or(self.window_position.0)
                }
                "window_position_y" => {
                    self.window_position.1 = value.parse().unwrap_or(self.window_position.1)
                }
                _ => {}
            }
        }
        ...
        Ok((
            config_path.file_name().unwrap().to_os_string(),
            config_content,
        ))
    }
...
pub struct Myxer {
    ...
    midi_controller_profiles: Shared<Option<MidiProfiles>>,
}
    ...
    pub fn new(app: &gtk::Application, pulse: &Shared<Pulse>) -> Self {
...

        {
            ...
            if meters.borrow().remember_position {
                window.move_(
                    meters.borrow().window_position.0,
                    meters.borrow().window_position.1,
                );
            }
        }

        {
            ...
            let midi_profiles = gtk::ModelButton::new();
            midi_profiles.set_property_text(Some("Midi Profiles"));
            midi_profiles.set_action_name(Some("app.midi_profiles"));
            prefs_box.add(&midi_profiles);
        ...
        let midi_controller_profiles = Shared::new(None);

        {
            ...
            let c_pulse = pulse.clone();
            let c_window = window.clone();
            let profiles = profiles.clone();
            card_profiles.connect_activate(move |_, _| {
                profiles.replace(Some(Profiles::new(&c_window, &c_pulse)));
            });
            actions.add_action(&card_profiles);

            let midi_profiles = gio::SimpleAction::new("midi_profiles", None);
            let m_pulse = pulse.clone();
            let m_window = window.clone();
            let profiles = midi_controller_profiles.clone();
            midi_profiles.connect_activate(move |_, _| {
            ...
            let split_channels = gio::SimpleAction::new_stateful(
                ...
                &(meters_clone.borrow().separate_channels).to_variant(),
            );
            ...
            let meters_clone = meters.clone();
            let remember_position = gio::SimpleAction::new_stateful(
                "remember_position",
                glib::VariantTy::new("bool").ok(),
                &(meters_clone.borrow().remember_position).to_variant(),
            );
            remember_position.connect_activate(move |s, _| {
                s.set_state(
                    &meters_clone
                        .borrow_mut()
                        .toggle_remember_position()
                        .to_variant(),
                )
            });
            ...
            let show_visualizers = gio::SimpleAction::new_stateful(
                "show_visualizers",
                glib::VariantTy::new("bool").ok(),
                &(meters_clone.borrow().show_visualizers).to_variant(),
            );
    ...
    pub fn update(&mut self) {
        ...
        if self.pulse.borrow_mut().update() {
            ...
            if let Some(sink) = pulse.sinks.get(&pulse.active_sink) {
                ...
                let refresh_peaks =
                    (meters.sink.peak != peak) || (meters.sink.split != separate && show);
            ...
            for (index, input) in &mut pulse.sink_inputs {
                ...
                if input.peak != 0 && Some(input.peak) == meter.peak {
                    if input.repetitions < REPETITIONS {
                        input.repetitions += 1;
                    } else {
                        if input.peak < DECREASE {
                            input.peak = 0;
                        } else {
                            input.peak -= DECREASE;
                        }
